...
void ScarfaceMenu::OnActivate()
{
	m_bIsActive ^= 1;
}

void ScarfaceMenu::OnToggleSlowMotion()
{
	if (m_bIsActive)
		return;
...
void ScarfaceMenu::OnToggleFreeCamera()
{
	if (m_bIsActive)
		return;
}
...
void ScarfaceMenu::OnPlayLastAnim()
{
	if (m_bIsActive)
		return;
...
void ScarfaceMenu::Draw()
{
	...
	if (ImGui::BeginMenuBar())
	{
		...
		if (ImGui::BeginMenu("Help"))
		{
			...
			{
				ImGui::MenuItem("Version: " SCARFACEHOOK_VERSION);
	...
	if (GetMainCharacter())
	{
		...
		if (ImGui::BeginTabItem("Script"))
		{
			...
			ImGui::InputTextMultiline("##notepad", console, sizeof(console), { -FLT_MIN, 150 });
...
void ScarfaceMenu::DrawSpawnerTab()
{
	...
	{
		ImGui::TextWrapped("Search");
		static ImGuiTextFilter vehFilter;
		ImGui::PushItemWidth(-FLT_MIN);
		vehFilter.Draw("##vf");
		ImGui::PopItemWidth();


		static int vehSelectID = 0;
		ImGui::BeginChild("##vehlist", { 0, 125.0f }, true);
		for (int n = 0; n < IM_ARRAYSIZE(szVehicles); n++)
		{
			if (vehFilter.PassFilter(szVehicles[n]))
			{
				bool is_selected = (vehSelectID == n);
				if (ImGui::Selectable(szVehicles[n], is_selected))
				{
					vehSelectID = n;
					sprintf(vehicle, "%s", szVehicles[vehSelectID]);
		...
		ImGui::EndChild();
		if (ImGui::Button("Create##vehicle", { -FLT_MIN, 0 }))
		{
	...
	{
		ImGui::TextWrapped("Search");
		static ImGuiTextFilter charFilter;
		ImGui::PushItemWidth(-FLT_MIN);
		charFilter.Draw("##cf");
		ImGui::PopItemWidth();


		static int charSelectID = 0;
		ImGui::BeginChild("##charlist", { 0, 125.0f }, true);
		for (int n = 0; n < IM_ARRAYSIZE(szPeds); n++)
		{
			if (charFilter.PassFilter(szPeds[n]))
			{
				bool is_selected = (charSelectID == n);
				if (ImGui::Selectable(szPeds[n], is_selected))
				{
					charSelectID = n;
					sprintf(characterSpawn, "%s", szPeds[charSelectID]);
...
void ScarfaceMenu::DrawWorldTab()
{
	...
	{
		GetTODObject()->SetTime(time[0], time[1]);
	}
	ImGui::Separator();
...
void ScarfaceMenu::DrawSpeedTab()
{
	...
	ImGui::PushItemWidth(-FLT_MIN);
	ImGui::SliderFloat("##speed",&m_fSpeed,0.0,5.0f);
	ImGui::PopItemWidth();
...
void ScarfaceMenu::DrawMiscTab()
{
	...
	{
		ImGui::TextWrapped("Search");
		static ImGuiTextFilter cutFilter;
		ImGui::PushItemWidth(-FLT_MIN);
		cutFilter.Draw("##csf");
		ImGui::PopItemWidth();


		static int cutSelectID = 0;
		ImGui::BeginChild("##cutlist", { 0, 125.0f }, true);
		for (int n = 0; n < IM_ARRAYSIZE(szCutscenes); n++)
		{
			if (cutFilter.PassFilter(szCutscenes[n]))
			{
				bool is_selected = (cutSelectID == n);
				if (ImGui::Selectable(szCutscenes[n], is_selected))
				{
					cutSelectID = n;
					sprintf(cutscene, "%s", szCutscenes[cutSelectID]);
		...
		ImGui::EndChild();
		if (ImGui::Button("Play##cutscene", { -FLT_MIN, 0 }))
		{
			playCutscene(cutscene);
		}
	}
	if (ImGui::CollapsingHeader("Missions"))
	{
		ImGui::TextWrapped("Search");
		static ImGuiTextFilter misFilter;
		ImGui::PushItemWidth(-FLT_MIN);
		misFilter.Draw("##msf");
		ImGui::PopItemWidth();


		static int misSelectID = 0;
		ImGui::BeginChild("##mislist", { 0, 125.0f }, true);
		for (int n = 0; n < IM_ARRAYSIZE(szMissions); n++)
		{
			if (misFilter.PassFilter(szMissions[n]))
			{
				bool is_selected = (misSelectID == n);
				if (ImGui::Selectable(szMissions[n], is_selected))
				{
					misSelectID = n;
					sprintf(mission, "%s", szMissions[misSelectID]);
...
{
	{
		...
		if (pat)
			Patch<char>(pat, (TheMenu->m_bIsActive || TheMenu->m_bAirbreak) ? 0xC3 : 0x64);
	}
	{
		uintptr_t pat = _pattern(PATID_DisableInput);
		if (pat)
			Patch<char>(pat, (TheMenu->m_bIsActive || TheMenu->m_bAirbreak || TheMenu->m_bFreeCam) ? 0x74 : 0x75);
	}
	...
	if (m_bAirbreak)
	{
		...
		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyForward))
			pos += fwd * TheMenu->m_fAirbreakSpeed * delta * 1;
		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyBack))
			pos += fwd * TheMenu->m_fAirbreakSpeed * delta * -1;

		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyLeft))
			pos += strafe * TheMenu->m_fAirbreakSpeed * delta * -1;
		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyRight))
			pos += strafe * TheMenu->m_fAirbreakSpeed * delta * 1;

		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyUp))
			pos += up * TheMenu->m_fAirbreakSpeed * delta * 1;
		if (GetAsyncKeyState(SettingsMgr->iFreeCameraKeyDown))
			pos += up * TheMenu->m_fAirbreakSpeed * delta * -1;
	...
	SetFallDamageValue(m_bDisableFallDamage ? 1000 : 0);
...
{
	{
		uintptr_t pat = _pattern(PATID_FallDamage1);
		if (pat)
			Patch<int>(pat, val);
	}
	{
		uintptr_t pat = _pattern(PATID_FallDamage2);
		if (pat)
			Patch<int>(pat, val);
	}
}
